...
def main(app, bcrypt_app):
    ...
    def check_password(form, field):
        ...
        query = """SELECT passwordhash
                   ...
                   WHERE username=?"""
        args = (username,)
        return bcrypt_app.check_password_hash(query_db(query, args)[0], password)

    def check_username(_, field):
        username = field.data
    ...
    class LoginForm(Form):
        ...
        password = PasswordField('password',
                                 validators=[InputRequired(), check_password])
        remember_me = BooleanField('remember_me', default=False)

    return login_manager, LoginForm
